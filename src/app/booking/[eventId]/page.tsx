...
const BookingPage = () => {
  ...
  const handleQuantityChange = (change: number) => {
    setQuantity((q) => Math.max(1, Math.min(10, q + change)));
  };
  ...
  const handleProceedToPayment = () => {
    if (userDetails.name && userDetails.email && userDetails.phone) setStep(2);
  };
  ...
  const generateSeatNumbers = (qty: number): string[] => {
    ...
    for (let i = 0; i < qty; i++) seats.push(`A${startSeat + i}`);
    return seats;
  };
                ...
                <div className="text-center">
                  <div className="bg-white rounded-2xl p-8 shadow-lg border border-gray-100">
                    ...
                    <p className="text-gray-600 mb-6">
                      Your tickets have been booked successfully. You will
